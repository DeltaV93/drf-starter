...
1. ``DJANGO_SETTINGS_MODULE=template.settings`` plus ``DJANGO_ENVIRONMENT``
   (development | production | testing). This is what manage.py, wsgi.py,
   asgi.py and celery.py default to.
2. ``DJANGO_SETTINGS_MODULE=template.settings.production`` -- name the
   environment module directly. This is what pytest and CI do.

The guard below matters: importing ``template.settings.testing`` also imports
...
if _SETTINGS_MODULE in ('', 'template.settings'):
